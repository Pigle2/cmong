...
### 33. `seller-dashboard.spec.ts` — 판매자 대시보드 강화 (6개)

> 판매자 대시보드 기획서 대비 UI 구현 검증. 배포 전에는 DASH-1~5 자동 스킵.

| ID | 테스트 | 로그인 | 설명 |
|----|--------|:------:|------|
| DASH-1 | 4개 주요 통계 카드 표시 | 판매자 | 이번 달 매출/진행중 주문/대기중 주문/등급 카드 존재 (미배포 시 스킵) |
| DASH-2 | 보조 지표 카드 (응답률, 평균 평점) 표시 | 판매자 | 2차 통계 카드 영역 응답률/평균 평점 (미배포 시 스킵) |
| DASH-3 | 할 일 섹션 표시 | 판매자 | "할 일" 섹션 존재 확인 (미배포 시 스킵) |
| DASH-4 | 최근 주문 섹션 표시 | 판매자 | "최근 주문" 섹션 존재 확인 (미배포 시 스킵) |
| DASH-5 | 전체보기 링크가 /seller/orders로 연결 | 판매자 | 최근 주문 "전체보기" href 검증 (미배포 시 스킵) |
| DASH-6 | 비인증 사용자는 로그인 페이지로 리다이렉트 | - | /seller/dashboard 비인증 접근 → /login 리다이렉트 |

---

### 34. `seller-orders-tabs.spec.ts` — 판매자 주문 관리 5탭 (7개)

> 판매자 주문 관리 2탭→5탭 확장 검증. 배포 전에는 SORD-1~6 자동 스킵.

| ID | 테스트 | 로그인 | 설명 |
|----|--------|:------:|------|
| SORD-1 | 5개 탭이 표시됨 | 판매자 | 대기/진행중/납품완료/완료/취소·분쟁 탭 존재 (미배포 시 스킵) |
| SORD-2 | 대기 탭이 기본 선택됨 | 판매자 | "대기" 탭 data-state="active" 확인 (미배포 시 스킵) |
| SORD-3 | 탭 클릭 시 해당 탭 콘텐츠 표시 | 판매자 | 완료 탭 클릭 → active 상태 전환 (미배포 시 스킵) |
| SORD-4 | 취소/분쟁 탭 클릭 시 표시 | 판매자 | 취소/분쟁 탭 클릭 → active 상태 전환 (미배포 시 스킵) |
| SORD-5 | 각 탭에 주문 수가 표시됨 | 판매자 | "대기 (N)" 형식 숫자 포함 검증 (미배포 시 스킵) |
| SORD-6 | 대기 주문에 수락/거절 버튼 존재 확인 | 판매자 | 대기 탭에 "주문 수락"/"주문 거절" 버튼 또는 빈 상태 메시지 (미배포 시 스킵) |
| SORD-7 | 비인증 사용자는 로그인 페이지로 리다이렉트 | - | /seller/orders 비인증 접근 → /login 리다이렉트 |

---

### 35. `seller-services-mgmt.spec.ts` — 판매자 서비스 관리 강화 (8개)

> 서비스 상태 탭 필터 + 복제/중지/재개/삭제 버튼 + 상태 변경 API. 배포 전에는 SVCM-1~7 자동 스킵.

| ID | 테스트 | 로그인 | 설명 |
|----|--------|:------:|------|
| SVCM-1 | "새 서비스 등록" 버튼 표시 | 판매자 | /seller/services/new 링크 존재 (미배포 시 스킵) |
| SVCM-2 | 상태 필터 탭 4개 표시 | 판매자 | 전체/판매중/임시저장/중지 탭 (미배포 시 스킵) |
| SVCM-3 | 전체 탭이 기본 선택됨 | 판매자 | data-state="active" 확인 (미배포 시 스킵) |
| SVCM-4 | 판매중 탭 클릭 시 전환 | 판매자 | 탭 클릭 → active 전환 (미배포 시 스킵) |
| SVCM-5 | 각 탭에 건수 표시 | 판매자 | "전체 (N)" 형식 확인 (미배포 시 스킵) |
| SVCM-6 | 서비스 카드에 수정 버튼 | 판매자 | 서비스 존재 시 수정 링크 또는 빈 상태 메시지 (미배포 시 스킵) |
| SVCM-7 | 서비스 카드에 삭제 버튼 | 판매자 | 서비스 존재 시 삭제 버튼 또는 빈 상태 메시지 (미배포 시 스킵) |
| SVCM-8 | 상태 변경 API 비인증 차단 | - | PATCH /api/seller/services/{id}/status 비인증 시 401/404 |

---

### 36. `seller-profile-enhanced.spec.ts` — 판매자 프로필 강화 (8개)

> 프로필 이미지/태그UI/경력/포트폴리오/인증 섹션. 배포 전에는 SPROF-1~8 자동 스킵.

| ID | 테스트 | 로그인 | 설명 |
|----|--------|:------:|------|
| SPROF-1 | 프로필 이미지 섹션 표시 | 판매자 | "프로필 이미지" 카드 존재 (미배포 시 스킵) |
| SPROF-2 | 이미지 변경 버튼 비활성화 | 판매자 | 프로토타입이므로 disabled 상태 (미배포 시 스킵) |
| SPROF-3 | 기본 정보 폼 필드 존재 | 판매자 | displayName/introduction 입력 필드 (미배포 시 스킵) |
| SPROF-4 | 전문 분야 태그 UI 표시 | 판매자 | "추가" 버튼으로 태그 추가 가능 (미배포 시 스킵) |
| SPROF-5 | 경력 사항 섹션 (프로토타입) | 판매자 | "추후 지원 예정" 안내 (미배포 시 스킵) |
| SPROF-6 | 포트폴리오 섹션 (프로토타입) | 판매자 | 포트폴리오 업로드 안내 (미배포 시 스킵) |
| SPROF-7 | 인증 정보 섹션 (프로토타입) | 판매자 | 본인/사업자 인증 미인증 상태 (미배포 시 스킵) |
| SPROF-8 | 저장 버튼 표시 | 판매자 | 저장 버튼 enabled 상태 (미배포 시 스킵) |

---

### 37. `seller-settlement.spec.ts` — 판매자 정산 관리 (7개)

> 정산 관리 프로토타입 페이지 UI 검증. 배포 전에는 SETL-1~6 자동 스킵.

| ID | 테스트 | 로그인 | 설명 |
|----|--------|:------:|------|
| SETL-1 | 정산 관리 타이틀 표시 | 판매자 | "정산 관리" 헤딩 (미배포 시 스킵) |
| SETL-2 | 프로토타입 안내 배너 표시 | 판매자 | "추후 지원 예정" 안내 (미배포 시 스킵) |
| SETL-3 | 3개 통계 카드 표시 | 판매자 | 가용 잔액/정산 대기/누적 수익 (미배포 시 스킵) |
| SETL-4 | 출금요청 버튼 비활성화 | 판매자 | disabled 상태 확인 (미배포 시 스킵) |
| SETL-5 | 정산 내역 테이블 헤더 | 판매자 | 일자/주문번호/금액/수수료/정산 (미배포 시 스킵) |
| SETL-6 | 계좌 미등록 + 등록 버튼 비활성화 | 판매자 | "미등록" 배지 + disabled 버튼 (미배포 시 스킵) |
| SETL-7 | 비인증 사용자 로그인 리다이렉트 | - | /seller/settlement 비인증 접근 → /login |

---

### 38. `seller-analytics.spec.ts` — 판매자 통계/분석 (7개)

> 통계/분석 프로토타입 페이지 UI 검증. 배포 전에는 ANLT-1~6 자동 스킵.

| ID | 테스트 | 로그인 | 설명 |
|----|--------|:------:|------|
| ANLT-1 | 통계/분석 타이틀 표시 | 판매자 | "통계/분석" 헤딩 (미배포 시 스킵) |
| ANLT-2 | 프로토타입 안내 배너 표시 | 판매자 | "추후 지원 예정" 안내 (미배포 시 스킵) |
| ANLT-3 | 기간 표시 (최근 30일) | 판매자 | 기간 뱃지 표시 (미배포 시 스킵) |
| ANLT-4 | 매출 통계 + 그래프 플레이스홀더 | 판매자 | 그래프 영역 + "추후 지원" 안내 (미배포 시 스킵) |
| ANLT-5 | 서비스별 통계 테이블 헤더 | 판매자 | 서비스명/조회수/주문수/전환율/매출 (미배포 시 스킵) |
| ANLT-6 | 리뷰 분석 섹션 | 판매자 | 평점 추이 + 주요 키워드 카드 (미배포 시 스킵) |
| ANLT-7 | 비인증 사용자 로그인 리다이렉트 | - | /seller/analytics 비인증 접근 → /login |

---

### 39. `seller-nav.spec.ts` — 판매자 네비게이션 (3개)

> 판매자 사이드바 네비게이션 7개 항목 검증. 배포 전에는 SNAV-1~3 자동 스킵.

| ID | 테스트 | 로그인 | 설명 |
|----|--------|:------:|------|
| SNAV-1 | 사이드바 7개 항목 표시 | 판매자 | 대시보드/서비스/주문/정산/통계/메시지/프로필 (미배포 시 스킵) |
| SNAV-2 | 정산 관리 링크 href 검증 | 판매자 | /seller/settlement 링크 (미배포 시 스킵) |
| SNAV-3 | 통계 링크 href 검증 | 판매자 | /seller/analytics 링크 (미배포 시 스킵) |

---

### 40. `review-filter-sort.spec.ts` — 리뷰 필터/정렬 (6개)

> 서비스 상세 리뷰 탭의 평점 필터 + 정렬 드롭다운 검증. 배포 전에는 RVF-1~6 자동 스킵.

| ID | 테스트 | 로그인 | 설명 |
|----|--------|:------:|------|
| RVF-1 | 리뷰 탭에 필터 버튼 표시 | - | 전체/5점/4점/3점/2점/1점 필터 버튼 (미배포 시 스킵) |
| RVF-2 | 정렬 드롭다운 표시 | - | 최신순/평점 높은순/평점 낮은순 셀렉트 (미배포 시 스킵) |
| RVF-3 | 평점 필터 클릭 시 필터링 동작 | - | 5점 클릭 → 필터 활성화 (미배포 시 스킵) |
| RVF-4 | 필터된 리뷰 수 표시 | - | "N개의 리뷰" 텍스트 (미배포 시 스킵) |
| RVF-5 | 빈 필터 결과 시 안내 메시지 | - | 1점 필터 시 빈 결과 처리 (미배포 시 스킵) |
| RVF-6 | 전체 필터로 복귀 시 모든 리뷰 표시 | - | 5점→전체 복귀 후 전체 리뷰 (미배포 시 스킵) |

---

### 41. `buyer-orders-tabs.spec.ts` — 구매자 주문 내역 상태 탭 (6개)

> 구매자 주문 목록 상태 필터 탭 검증. 배포 전에는 BORD-1~5 자동 스킵.

| ID | 테스트 | 로그인 | 설명 |
|----|--------|:------:|------|
| BORD-1 | 4개 탭 표시 (전체/진행중/완료/취소) | 구매자 | 탭 4개 존재 (미배포 시 스킵) |
| BORD-2 | 전체 탭이 기본 선택됨 | 구매자 | data-state="active" 확인 (미배포 시 스킵) |
| BORD-3 | 진행중 탭 클릭 시 전환 | 구매자 | 탭 전환 → active (미배포 시 스킵) |
| BORD-4 | 완료 탭 클릭 시 전환 | 구매자 | 탭 전환 → active (미배포 시 스킵) |
| BORD-5 | 각 탭에 건수 표시 | 구매자 | "전체 (N)" 형식 확인 (미배포 시 스킵) |
| BORD-6 | 비인증 사용자 로그인 리다이렉트 | - | /orders 비인증 접근 → /login |
...
| 판매자 대시보드 강화 | seller-dashboard.spec.ts | 6 | 매출/대기중/할일/최근주문/전체보기/비인증 리다이렉트 |
| 판매자 주문 5탭 | seller-orders-tabs.spec.ts | 7 | 5탭 표시/기본 대기/탭전환/수락거절 버튼/비인증 리다이렉트 |
| 판매자 서비스 관리 강화 | seller-services-mgmt.spec.ts | 8 | 상태탭/건수/수정삭제 버튼/상태변경 API |
| 판매자 프로필 강화 | seller-profile-enhanced.spec.ts | 8 | 이미지/태그/경력/포트폴리오/인증 섹션 |
| 판매자 정산 관리 | seller-settlement.spec.ts | 7 | 통계카드/출금/테이블/계좌 (프로토타입) |
| 판매자 통계/분석 | seller-analytics.spec.ts | 7 | 매출그래프/서비스통계/리뷰분석 (프로토타입) |
...
| 판매자 관리 | `seller.spec.ts`, `seller-dashboard.spec.ts`, `seller-orders-tabs.spec.ts`, `seller-services-mgmt.spec.ts`, `seller-profile-enhanced.spec.ts`, `seller-settlement.spec.ts`, `seller-analytics.spec.ts`, `seller-nav.spec.ts` |
| 리뷰 | `review.spec.ts` |
